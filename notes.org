* TODO [0/15]
  1. [ ] tidy up js side when it receives 0 vertices to render
  2. [ ] interp_fn_id equivalents + error messages when missing
  3. [ ] test gc
  4. [ ] check if minus numbers work? (should do)
  5. [ ] look into replacing append_to_vector with append_heap_var_to_vector
  6. [ ] poly should also accept a uv parameter
  7. [ ] use the f32_array to pack in the values required by the prng and interp structs
  8. [ ] ensure that there's a dist folder before running npm run build:wasm
  9. [ ] test new setup/teardown code
