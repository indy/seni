...
#define SEN_POOL(ITEM, ITEM_NAME)                                              \
  ...
  bool ITEM_NAME##_pool_add_slab(struct ITEM##_pool* ITEM_NAME##_pool) {       \
    if (ITEM_NAME##_pool->num_slabs >= ITEM_NAME##_pool->max_slabs_allowed) {  \
      ...
      SEN_ERROR("will not allocate more than %d %s",                           \
                ITEM_NAME##_pool->max_slabs_allowed, slab_name);               \
      return false;                                                            \
    }                                                                          \
                                                                               \
    struct ITEM##_slab* ITEM_NAME##_slab =                                     \
  ...
  void ITEM_NAME##_pool_free(struct ITEM##_pool* ITEM_NAME##_pool) {           \
    struct ITEM##_slab* ITEM_NAME##_slab =                                     \
        ITEM_NAME##_pool->ITEM_NAME##_slabs;                                   \
  ...
  ITEM* ITEM_NAME##_pool_get(struct ITEM##_pool* ITEM_NAME##_pool) {           \
    if (ITEM_NAME##_pool->available == NULL) {                                 \
      if (!ITEM_NAME##_pool_add_slab(ITEM_NAME##_pool)) {                      \
        const char* slab_name = #ITEM_NAME "_slabs";                           \
        SEN_ERROR("cannot add more than %d %s",                                \
                  ITEM_NAME##_pool->max_slabs_allowed, slab_name);             \
        return NULL;                                                           \
      }                                                                        \
    }                                                                          \
    ...
    if (ITEM_NAME##_pool->current_water_mark >                                 \
    ...
    return head;                                                               \
                                                                               ...
                                                                               \
  void ITEM_NAME##_pool_return(struct ITEM##_pool* ITEM_NAME##_pool,           \
                               ITEM*               ITEM_NAME) {                              \
    ITEM_NAME->next = NULL;                                                    \
    ITEM_NAME->prev = NULL;                                                    \
                                                                               \
  ...
  void ITEM_NAME##_pool_pretty_print(struct ITEM##_pool* ITEM_NAME##_pool) {   \
    ...
    SEN_LOG("%s_pool:"                                                         \
            ...
            item_name, ITEM_NAME##_pool->slab_size,                            \
            ITEM_NAME##_pool->num_slabs, ITEM_NAME##_pool->max_slabs_allowed,  \
            ITEM_NAME##_pool->get_count, ITEM_NAME##_pool->return_count,       \
            ITEM_NAME##_pool->high_water_mark,                                 \
            ITEM_NAME##_pool->current_water_mark);                             \
  }
