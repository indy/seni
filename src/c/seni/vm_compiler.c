...
i32 add_internal_local_mapping(seni_program* program) {
  ...
  SENI_ERROR("add_internal_local_mapping failed: increase MEMORY_LOCAL_SIZE from %d",
             MEMORY_LOCAL_SIZE);
  return -1;
}
  ...
  for (i32 i = 0; i < program->code_size; i++) {
    ...
    if (bc->op == CALL) {
      ...
      if (offset->op != LOAD && offset->arg0.value.i != MEM_SEG_CONSTANT) {
        SENI_ERROR("correct_function_addresses expected a 'LOAD CONST' 2 "
      ...
      offset = bc - 1;
      if (offset->op != LOAD && offset->arg0.value.i != MEM_SEG_CONSTANT) {
        SENI_ERROR("correct_function_addresses expected a 'LOAD CONST' 1 "
