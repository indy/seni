...
* VAR_2D [7/9]
  1. [X] update the compiler
  2. [X] modify PILE, APPEND and NTH to also account for VAR_2D
  3. [X] READ_STACK_ARG_VAR
  4. [X] READ_STACK_ARG_VEC2 (bind macros should expect a VAR_2D)
  5. [X] update void var_as_vec2(f32 *out0, f32 *out1, seni_var *var)
  6. [X] look into void var_as_vec4(f32 *out0, f32 *out1, f32 *out2, f32 *out3, seni_var *var)
  7. [X] check DEC_RC, INC_RC
  8. [ ] update ADD, SUB, MUL, DIV, NEG
  9. [ ] check EQ, LT, GT

(define vec2d [4 5]) vec2d
