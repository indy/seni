...
function senieMode() {
  ...
  const SENIECOMMON = 'senie-common';
  const PARAMETER = 'senie-parameter';
  ...
  // keywords are core to the senie language
  const keywords =
  ...
  // functions from the common senie library
  const senieCommon = makeKeywords(`* + - / < = > append begin bezier
    ...
    token: (stream, state) => {
      ...
      default: // default parsing mode
        ...
        if (ch === '\"') {
          ...
          } else if (senieCommon.propertyIsEnumerable(stream.current())) {
            returnType = tokenType(SENIECOMMON, state);
...
const CodeMirrorSenie = {
  senieMode
};

export default CodeMirrorSenie;
