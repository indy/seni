...
senie_matrix g_identity;
senie_colour g_unseen_colour;
...
void add_vertex(senie_render_packet* render_packet,
                senie_matrix*        matrix,
                f32                  x,
                f32                  y,
                senie_colour*        rgb,
...
void form_degenerate_triangle(senie_render_packet* render_packet,
                              senie_matrix*        matrix,
                              f32                  x,
                              f32                  y) {
...
bool can_vertices_fit(senie_render_packet* render_packet, i32 num, i32 max_vertices) {
  return render_packet->num_vertices < (max_vertices - (num + 2));
}

bool is_render_packet_empty(senie_render_packet* render_packet) {
  return render_packet->num_vertices == 0;
}

void prepare_to_add_triangle_strip(senie_render_data* render_data,
                                   senie_matrix*      matrix,
                                   i32                num_vertices,
                                   f32                x,
                                   f32                y) {
  ...
  senie_render_packet* render_packet = render_data->current_render_packet;
...
void render_line(senie_render_data* render_data,
                 ...
                 f32                width,
                 senie_colour*      colour) {
  senie_uv_mapping* uv = get_uv_mapping(BRUSH_FLAT, 0, true);
...
void render_circle_slice(senie_render_data* render_data,
  ...
  if (angle_start > angle_end) {
    SENIE_LOG("angle-start (%.2f) > angle-end (%.2f) ???", angle_start, angle_end);
  }
...
void render_poly(senie_render_data* render_data,
  ...
  senie_var* coord  = coords->value.v;
  senie_var* colour = colours->value.v;

  senie_colour rgb_colour, other_colour;
...
void render_quadratic(senie_render_data* render_data,
  ...
  //
  senie_brush_type  brush_type = (senie_brush_type)(brush - INAME_BRUSH_FLAT);
  senie_uv_mapping* uv         = get_uv_mapping(brush_type, brush_subtype, true);
...
void render_stroked_bezier(senie_render_data* render_data,
  ...
  for (i32 i = 0; i < tessellation; i++) {
    ...
    lab.element[0] = lab_l + (senie_perlin(xx1, xx1, xx1) * colour_volatility);

    quad_coords[0] = xx1 + (ns * senie_perlin(xx1, xx1, seed));
    quad_coords[1] = yy1 + (ns * senie_perlin(yy1, yy1, seed));
    quad_coords[2] = xx2 + (ns * senie_perlin(xx2, xx1, seed));
    quad_coords[3] = yy2 + (ns * senie_perlin(yy2, yy1, seed));
    quad_coords[4] = xx3 + (ns * senie_perlin(xx3, xx1, seed));
    quad_coords[5] = yy3 + (ns * senie_perlin(yy3, yy1, seed));
...
void render_stroked_bezier_rect(senie_render_data* render_data,
  ...
  for (i = 0; i < iiterations; i++) {
    ...
    coords[0] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + x_start + (0 * th_width);
    coords[1] = h + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);

    coords[2] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + x_start + (1 * th_width);
    coords[3] = h + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);

    coords[4] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + x_start + (2 * th_width);
    coords[5] = h + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);

    coords[6] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + x_start + (3 * th_width);
    coords[7] = h + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);
  ...
  for (i = 0; i < iiterations; i++) {
    ...
    coords[0] = v + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);
    coords[1] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + y_start + (0 * th_height);

    coords[2] = v + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);
    coords[3] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + y_start + (1 * th_height);

    coords[4] = v + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);
    coords[5] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + y_start + (2 * th_height);

    coords[6] = v + (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol);
    coords[7] = (senie_prng_f32_range(&prng_state, -1.0f, 1.0f) * vol) + y_start + (3 * th_height);
