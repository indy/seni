...
* building for seni.app/gallery
  ...
  1. set the loadForWebsite variable in src/js/app-piece.js to true
  2. set the loadForWebsite variable in src/js/worker.js to true
  3. delete the dist folder
  4. $ npm run build:production
  5. $ npm run build:wasm
  6. copy the following files into the seni.app/static/seni folder:
     - dist/piece.bundle.js
     - dist/sen-wasm.wasm
