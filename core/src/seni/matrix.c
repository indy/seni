...
void matrix_ortho(sen_matrix* out, f32 left, f32 right, f32 bottom, f32 top, f32 near,
                  f32 far) {
...
void matrix_transform_vec3(f32* outx, f32* outy, f32* outz, sen_matrix* m, f32 x, f32 y,
                           f32 z) {
...
void matrix_stack_reset(sen_matrix_stack* matrix_stack) {
  ...
  sen_matrix* m = &(matrix_stack->stack[0]);
  matrix_identity(m);
...
sen_matrix* matrix_stack_push(sen_matrix_stack* matrix_stack) {
  if (matrix_stack->sp == MATRIX_STACK_SIZE) {
    SEN_ERROR("matrix_stack_push: matrix stack is full");
    return NULL;
  }

  sen_matrix* old_top = &(matrix_stack->stack[matrix_stack->sp - 1]);
  sen_matrix* new_top = &(matrix_stack->stack[matrix_stack->sp]);
...
sen_matrix* matrix_stack_pop(sen_matrix_stack* matrix_stack) {
  if (matrix_stack->sp == 0) {
    SEN_ERROR("matrix_stack_pop: matrix stack is empty");
    return NULL;
  }

  matrix_stack->sp--;
  sen_matrix* m = &(matrix_stack->stack[matrix_stack->sp]);

  return m;
}

sen_matrix* matrix_stack_peek(sen_matrix_stack* matrix_stack) {
  sen_matrix* head = &(matrix_stack->stack[matrix_stack->sp - 1]);

  return head;
}

void matrix_stack_scale(sen_matrix_stack* matrix_stack, f32 sx, f32 sy) {
  sen_matrix* m = matrix_stack->wip_transform;
  matrix_identity(m);
...
void matrix_stack_transform_vec2(f32* outx, f32* outy, sen_matrix_stack* matrix_stack, f32 x,
                                 f32 y) {
  sen_matrix* head = matrix_stack_peek(matrix_stack);
...
void matrix_stack_transform_vec3(f32* outx, f32* outy, f32* outz,
                                 sen_matrix_stack* matrix_stack, f32 x, f32 y, f32 z) {
  sen_matrix* head = matrix_stack_peek(matrix_stack);
