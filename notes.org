* TODO [0/13]
  1. [ ] 151c-flower - bezier-bulging
  2. [ ] 154c-stroked-bezier - stroked-bezier-rect
  3. [ ] 1556-biomorphs - repeat/symmetry-vertical - takes a 'draw' argument
  4. [ ] 155j-chaotic-grid-2 - focal/point
  5. [ ] 1560-path - make a function ???
  6. [ ] 157e-tile-shadow - col/procedural-fn
  7. [ ] 15fe-rotate-mirror - poly, repeat/rotate-mirrored
  8. [ ] cleanup memory of parser
  9. [ ] structs for compile, runtim, performance info
  10. [ ] seni_env
  11. [ ] 'in' paramaeter for loop ?
  12. [ ] remove hsv colour space? - would this get rid of the illegal state?
  13. [ ] should ADD, SUB, MUL, DIV, NEG be updated to take into account VAR_2D?
...
* calling functions
  ...
  need
  - INVOKE, INVOKE_0
  - a way of pushing the iname of a function onto the stack and into variables etc
  - vm needs access to fn_info

  --------------------------------------------------------------------------------

  INVOKE an equivalent to CALL but for when the called function isn't known at compile time.
  instead the function's iname will be pushed onto the stack
  will also need an INVOKE_0
