* TODO [1/18]
  ...
  2. [ ] impl: random_crossover(seni_genotype *a, seni_genotype *b, i32 genotype_length)
  3. [ ] impl: initial seed
  4. [ ] check buffer sizes and pool usage during a typical render/evolve session
  5. [ ] add an END token to serialized output, use as a sanity check
  6. [ ] share codegen preamble between traits
  7. [ ] add unit tests for compiling with a genotype
  8. [ ] remove Immutable
  9. [ ] get high-res render working
  10. [ ] pretty print program emits names
  11. [ ] better seni_ga.c::hack_node_to_var (lists, 2d etc)
  12. [ ] unify args to *_pretty_print
  13. [ ] maybe have a required specifier in functions?
  14. [ ] poly should also accept a uv parameter
  15. [ ] ensure that there's a dist folder before running npm run build:wasm
  16. [ ] remove hsv colour space? - would this get rid of the illegal state?
