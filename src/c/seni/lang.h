...
// which value to use from the unions that are specified in both seni_node and
// seni_var
typedef enum {
  USE_UNKNOWN,
  USE_I,          // integer
  ...
  USE_SRC,        // (seni_node only) pointer to original source (for whitespace +
                  // comments)
...
struct seni_vm {
  ...
  seni_var *heap_avail;          // doubly linked list of unallocated seni_vars from the
                                 // heap_slab
