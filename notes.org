* TODO [3/14]
  1. [ ] mark and sweep gc
  2. [ ] use the f32_array to pack in the values required by the prng and interp structs
  ...
  12. [X] sqrt - and fix up hex-grid
  13. [X] make sure that the heap is fully restored in between running scripts
  14. [X] colour presets
