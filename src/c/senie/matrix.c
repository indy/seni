...
senie_matrix_stack* matrix_stack_allocate() {
  senie_matrix_stack* matrix_stack = (senie_matrix_stack*)calloc(1, sizeof(senie_matrix_stack));

  matrix_stack->stack_size = MATRIX_STACK_SIZE;
  matrix_stack->stack      = (senie_matrix*)calloc(MATRIX_STACK_SIZE, sizeof(senie_matrix));

  matrix_stack->wip_transform = (senie_matrix*)calloc(1, sizeof(senie_matrix));
...
void matrix_stack_reset(senie_matrix_stack* matrix_stack) {
  ...
  senie_matrix* m = &(matrix_stack->stack[0]);
  matrix_identity(m);
...
senie_matrix* matrix_stack_push(senie_matrix_stack* matrix_stack) {
  if (matrix_stack->sp == MATRIX_STACK_SIZE) {
    SENIE_ERROR("matrix_stack_push: matrix stack is full");
    return NULL;
  }

  senie_matrix* old_top = &(matrix_stack->stack[matrix_stack->sp - 1]);
  senie_matrix* new_top = &(matrix_stack->stack[matrix_stack->sp]);
...
senie_matrix* matrix_stack_pop(senie_matrix_stack* matrix_stack) {
  if (matrix_stack->sp == 0) {
    SENIE_ERROR("matrix_stack_pop: matrix stack is empty");
    return NULL;
  }

  matrix_stack->sp--;
  senie_matrix* m = &(matrix_stack->stack[matrix_stack->sp]);

  return m;
}

senie_matrix* matrix_stack_peek(senie_matrix_stack* matrix_stack) {
  senie_matrix* head = &(matrix_stack->stack[matrix_stack->sp - 1]);

  return head;
}

void matrix_stack_scale(senie_matrix_stack* matrix_stack, f32 sx, f32 sy) {
  senie_matrix* m = matrix_stack->wip_transform;
  matrix_identity(m);
  matrix_scale(m, m, sx, sy, 1.0f);

  senie_matrix* head = matrix_stack_peek(matrix_stack);
