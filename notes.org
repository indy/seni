* TODO [0/20]
  1. [ ] app could have a +- controls for resizing the preview canvas (looks small on a big monitor)
  2. [ ] poly-line to complement poly
  3. [ ] line could take a per-vertex colour
  4. [ ] var_copy doesn't copy mark. is this intentional?
  5. [ ] path/linear: take t-start/t-end into account
  6. [ ] program_allocate uses calloc, make sure program_free is called
  7. [ ] bind_gen_col set min/max according to colour format
  8. [ ] 161g-grid-flow (shows evolve mismatched rendering error)
  9. [ ] quilt
  10. [ ] map (1669-zero.sen)
  11. [ ] better error messages when gen/scalar was mis-spelt
  12. [ ] {red (gen/col alpha: 1)} bug?
  13. [ ] poly should also accept a uv parameter
  14. [ ] remove vm/env from genotype_list_create_initial_generation
  15. [ ] impl: initial seed
  16. [ ] add an END token to serialized output, use as a sanity check?
  17. [ ] add unit tests for compiling with a genotype
  18. [ ] ensure that there's a dist folder before running npm run build:wasm
  19. [ ] look at colour palettes in https://www.npmjs.com/package/nice-color-palettes
  20. [ ] look into restrict keyword in C99
...
** Rich Colour
   This means an abundance of gradients within color – they are called "valeurs" in painting.
   ...
   Surprising but true – the more saturated a photo is, the less nuanced and subtle its color.
