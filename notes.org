* TODO [2/12]
  1. [ ] use the f32_array to pack in the values required by the prng and interp structs
  2. [ ] ensure that there's a dist folder before running npm run build:wasm
  3. [ ] test new setup/teardown code
  4. [ ] cleanup memory of parser
  5. [ ] structs for compile, runtim, performance info
  6. [ ] seni_env
  7. [ ] 'in' parameter for loop ?
  ...
  11. [X] make sure that the heap is fully restored in between running scripts
  12. [X] colour presets
