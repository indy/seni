* TODO [1/20]
  1. [ ] gen/vector & rename to gen/2d-vector
  2. [ ] check usage of warn_if_alterable
  ...
  6. [ ] separate CodeMirror out from the build, load in it's own script tag
  7. [ ] remove Immutable
  ...
  13. [ ] better seni_ga.c::hack_node_to_var (lists, 2d etc)
  14. [ ] unify args to *_pretty_print
  15. [ ] maybe have a required specifier in functions?
  16. [ ] poly should also accept a uv parameter
  17. [ ] ensure that there's a dist folder before running npm run build:wasm
  18. [ ] remove hsv colour space? - would this get rid of the illegal state?
