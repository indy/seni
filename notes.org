...
* replacing emscripten with webassembly-npm

** -i app/c/seni_bind.c
   memcpy due to bind functions returning a seni_var struct, change this to return a pointer

   declare native_function_ptr as
   typedef seni_var *(*native_function_ptr)(seni_vm *vm, i32 num_args);

** -i app/c/seni_colour.c
 powf -> Math_pow   fixes problem


** -i app/c/seni_lang.c
 __floatscan error due to strtof in consume_float(char **src)
 ...
 __assert_fail due to DL_DELETE

 deleting the 2 asserts in utlist.h fixes this 

 -------------------------------------------------- 



-i app/c/seni_bind.c -i app/c/seni_lang.c
