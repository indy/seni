* TODO [2/10]
  ...
  3. [ ] structs for compile, runtim, performance info
  4. [ ] seni_env
  5. [ ] 'in' parameter for loop ?
  6. [ ] remove hsv colour space? - would this get rid of the illegal state?
  7. [ ] should ADD, SUB, MUL, DIV, NEG be updated to take into account VAR_2D?
  8. [ ] look at colour palettes in https://www.npmjs.com/package/nice-color-palettes
  9. [X] make sure that the heap is fully restored in between running scripts
  10. [X] colour presets

* updating scripts [9/57]
