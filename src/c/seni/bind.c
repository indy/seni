...
#define IS_F32(n)                            \
  if (value_1->type != VAR_FLOAT) {          \
    SENI_ERROR("expected f32 for: %s", #n);  \
    var_pretty_print("received: ", value_1); \
  }
#define IS_I32(n)                            \
  if (value_1->type != VAR_INT) {            \
    SENI_ERROR("expected i32 for: %s", #n);  \
    var_pretty_print("received: ", value_1); \
  }
#define IS_NAME(n)                           \
  if (value_1->type != VAR_NAME) {           \
    SENI_ERROR("expected name for: %s", #n); \
    var_pretty_print("received: ", value_1); \
  }
#define IS_COL(n)                              \
  if (value_1->type != VAR_COLOUR) {           \
    SENI_ERROR("expected colour for: %s", #n); \
    var_pretty_print("received: ", value_1);   \
  }
#define IS_LONG(n)                           \
  if (value_1->type != VAR_LONG) {           \
    SENI_ERROR("expected long for: %s", #n); \
    var_pretty_print("received: ", value_1); \
  }
#else
#define IS_F32
...
seni_var *bind_col_convert(seni_vm *vm, i32 num_args) {
  ...
  // the seni_var referencing the converted colour is going to be added to the
...
seni_var *bind_prng_build(seni_vm *vm, i32 num_args) {
  ...
  // build a seni_prng_state and call it once - this always returns 0 but
...
seni_var *bind_parametric_cos(seni_vm *vm, i32 num_args) {
  ...
  // SENI_PRINT("amp %.2f, freq %.2f, t %.2f value = %.2f", amplitude,
  // frequency, t, value);
  return &g_var_scratch;
}
