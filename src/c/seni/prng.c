...
void sen_prng_copy(sen_prng_state* dest_prng_state, sen_prng_state* src_prng_state) {
...
f32 sen_prng_f32_range(sen_prng_state* prng_state, f32 min, f32 max) {
  f32 value = sen_prng_f32(prng_state);
  value     = (value * (max - min)) + min;

  return value;
}

f32 sen_prng_f32_around(sen_prng_state* prng_state, f32 val, f32 percent, f32 min, f32 max) {
  f32 value   = sen_prng_f32(prng_state);
  ...
  // SEN_LOG("sen_prng_f32_around: res: %.2f, val: %.2f, percent: %.2f, min:
  // %.2f, max: %.2f", res, val, percent, min, max);
...
// -1..1
f32 sen_perlin(f32 x, f32 y, f32 z) { return noise(x, y, z); }
