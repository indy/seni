...
#define SENIE_PRINTF_BUFFER_SIZE 500

void senie_printf(char const* fmt, ...) {
  char buffer[SENIE_PRINTF_BUFFER_SIZE];

  va_list va;
  va_start(va, fmt);
  stbsp_vsnprintf(buffer, SENIE_PRINTF_BUFFER_SIZE, fmt, va);
  va_end(va);
...
void senie_fileline_sprintf(char*       buf,
                            int         buffer_size,
                            char*       file,
                            int         line,
                            char const* fmt,
...
void senie_printf_log(char* file, int line, char const* fmt, ...) {
  ...
  va_list va;
  va_start(va, fmt);
  senie_fileline_sprintf(buffer, SENIE_PRINTF_BUFFER_SIZE, file, line, fmt, va);
  va_end(va);
