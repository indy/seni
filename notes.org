* TODO [0/14]
  1. [ ] test gc
  2. [ ] replace ?/call with ?/invoke call: foo
  3. [ ] look into replacing append_to_vector with append_heap_var_to_vector
  4. [ ] poly should also accept a uv parameter
  5. [ ] call and fn naming convention - also use same arg naming
  6. [ ] use the f32_array to pack in the values required by the prng and interp structs
  7. [ ] ensure that there's a dist folder before running npm run build:wasm
  8. [ ] test new setup/teardown code
