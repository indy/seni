...
sen_node* assert_parser_node_txt(sen_node* node, sen_node_type type, char* val,
...
void test_genotype_stray_2d(void) {
...

  {
    ...
    assert_sen_var_f32(v, VAR_FLOAT, 106.014359f);

    g = g->next;
    v = g->var;

    assert_sen_var_f32(v, VAR_FLOAT, 207.37352f);
...
void test_unparser(void) {
  ...
  unparse_compare(9875, "(+ 6 {3 (gen/int min: 1 max: 50)})",
  ...
  unparse_compare(6534, "{3.4 (gen/scalar min: 0 max: 9)}",
                  "{7.5 (gen/scalar min: 0 max: 9)}");

  unparse_compare(6534, "(col/rgb r: {0.4 (gen/scalar)} g: 0.1)",
                  "(col/rgb r: {0.8 (gen/scalar)} g: 0.1)");
...
void test_unparser_vectors(void) {
  unparse_compare(9999, "{[[1.00 2.00] [3.00 4.00]] (gen/2d)}",
...
void test_unparser_single_trait_vectors(void) {
  unparse_compare(1298, "{[10 20] (gen/stray-2d from: [10 20] by: [5 5])}",
                  "{[13 19] (gen/stray-2d from: [10 20] by: [5 5])}");
}

void test_unparser_multiple_floats(void) {
  unparse_compare(3455, "{[0.977 0.416 0.171] (gen/scalar)}",
...
void test_serialization_genotype(void) {
...

  {
    ...
    genotype = genotype_construct(
        7534, "(+ {2 (gen/int min: 1 max: 30)} {5 (gen/int min: 1 max: 30)})");
    TEST_ASSERT(genotype);
...
void test_serialization_genotype_list(void) {
...

  {
    ...
    genotype = genotype_construct(
        2313, "(+ {2 (gen/int min: 1 max: 30)} {5 (gen/int min: 1 max: 30)})");
    TEST_ASSERT(genotype);
    genotype_list_add_genotype(genotype_list, genotype);

    genotype = genotype_construct(
        4562, "(+ {2 (gen/int min: 1 max: 30)} {5 (gen/int min: 1 max: 30)})");
    TEST_ASSERT(genotype);
    genotype_list_add_genotype(genotype_list, genotype);
...
void bug_f32_expr_with_genotype(void) {
  // vm_compile_f32_with_2_genes("(define v {[150 250] (gen/scalar min: 10 max:
  // 99)}) (nth from: v n: 0)",
  //                             1111, 78.578339f, 78.578339f, 49.573952f);

  vm_compile_f32_with_2_genes(
      "(define v {[364.374 334.649] (gen/stray-2d from: [100 200] by: [20 "
      "20])}) (nth from: v n: 0)",
      1111, 110.821724f, 110.821724f, 197.786041f);
  vm_compile_f32_with_2_genes(
      "(define v {[364.374 334.649] (gen/stray-2d from: [100 200] by: [20 "
