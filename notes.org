* TODO [3/11]
  1. [X] add checks to parsing code in bind
  2. [X] add checks about number of local variables declared
  3. [X] add checks about number of global variables declared
  4. [ ] functions will not be nestable
  5. [ ] manipulate vectors
  6. [ ] canvas/width canvas/height bind to global mappings
  7. [ ] seni_env
  8. [ ] interp/fn interp/bezier
  9. [ ] 'in' paramaeter for loop ?
  10. [ ] how would a stack machine compile first class functions? could there be alternatives?
