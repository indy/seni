...
#define STACK_PUSH                                       \
  ...
  sp++;                                                  \
  if (sp == stack_size) {                                \
    SENI_ERROR("Reached stack limit of %d", stack_size); \
    return false;                                        \
  }
  ...
  for (;;) {
    ...
    case SQUISH2:
      ...
      if (v->type != VAR_FLOAT) {
        ...
        // was the seni code declaring a vector of length 2 that didn't contain
    ...
    case STORE_F:
      ...
      if (memory_segment_type == MEM_SEG_ARGUMENT) {
        ...
        SENI_ERROR("STORE_F: should only be used with MEM_SEG_ARGUMENT, not %d",
                   memory_segment_type);
      }
      break;
    case STOP:
