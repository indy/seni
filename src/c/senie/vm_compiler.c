...
senie_program* get_preamble_program() { return g_preamble_program; }

void gene_assign_to_node(senie_genotype* genotype, senie_node* node) {
  if (node->alterable) {
    if (node->type == NODE_VECTOR) {
      // grab a gene for every element in this vector
      for (senie_node* n = safe_first_child(node); n != NULL; n = safe_next(n)) {
    ...
    if (get_node_value_in_use(node->type) == USE_FIRST_CHILD) {
      senie_node* first_child = safe_first(node->value.first_child);
      if (first_child) {
...
bool genotype_assign_to_ast(senie_genotype* genotype, senie_node* ast) {
  ...
  if (gene != NULL) {
    SENIE_ERROR("genotype_assign_to_ast: genes remaining after assigning "
                "genotype to ast");
    return false;
  }
...
i32 get_node_value_i32_from_gene(senie_node* node) {
  senie_gene* gene = node->gene;
  if (gene == NULL) {
    SENIE_ERROR("null gene returned");
    return 0;
  }
...
f32 get_node_value_f32_from_gene(senie_node* node) {
  senie_gene* gene = node->gene;
  if (gene == NULL) {
    SENIE_ERROR("null gene returned");
    return 0.0f;
...
bool alterable(senie_node* node) { return node->alterable && g_use_genes; }

i32 get_node_value_i32(senie_node* node) {
  if (alterable(node)) {
...
f32 get_node_value_f32(senie_node* node) {
  if (alterable(node)) {
...
void warn_if_alterable(char* msg, senie_node* node) {
  if (node->alterable) {
    SENIE_ERROR("warn_if_alterable: %s", msg);
  }
}
...
program_emit_opcode(senie_program* program, senie_opcode op, senie_var* arg0, senie_var* arg1) {
  ...
  senie_bytecode* b = &(program->code[program->code_size++]);
  b->op             = op;
...
i32 add_local_mapping(senie_program* program, i32 word_lut_value) {
  ...
  SENIE_ERROR("add_local_mapping failed: increase MEMORY_LOCAL_SIZE from %d", MEMORY_LOCAL_SIZE);
  return -1;
}
...
i32 add_internal_local_mapping(senie_program* program) {
  ...
  SENIE_ERROR("add_internal_local_mapping failed: increase MEMORY_LOCAL_SIZE from %d",
              MEMORY_LOCAL_SIZE);
  return -1;
}
...
i32 add_global_mapping(senie_program* program, i32 word_lut_value) {
  ...
  SENIE_ERROR("add_global_mapping failed: increase MEMORY_GLOBAL_SIZE from %d", MEMORY_GLOBAL_SIZE);
  return -1;
}
...
i32 get_fn_info_index(senie_node* node, senie_program* program) {
  if (node->type != NODE_NAME) {
    SENIE_ERROR("get_fn_info_index not given a name node");
  ...
  SENIE_ERROR("get_fn_info_index unable to find fn_info for a function");
  return -1;
}

senie_fn_info* get_fn_info(senie_node* node, senie_program* program) {
  if (node->type != NODE_NAME) {
    return NULL;
  }
...
i32 node_vector_length(senie_node* vector_node) {
  i32 length = 0;
  for (senie_node* node = safe_first(vector_node->value.first_child); node != NULL;
       node             = safe_next(node)) {
    length++;
  }
  return length;
...
bool all_children_have_type(senie_node* parent, senie_node_type type) {
  if (parent->type != NODE_VECTOR && parent->type != NODE_LIST) {
    SENIE_ERROR("all_children_have_type need a vector or list");
    return false;
  }

  senie_node* child = parent->value.first_child;
  while (child != NULL) {
    if (child->type != type) {
      return false;
    }
...
i32 count_children(senie_node* parent) {
  if (parent->type != NODE_VECTOR && parent->type != NODE_LIST) {
    SENIE_ERROR("count_children need a vector or list");
    return 0;
  }

  i32         count = 0;
  senie_node* child = safe_first(parent->value.first_child);
  while (child != NULL) {
    count++;
    child = safe_next(child);
  }
    ...
    if (lhs_node->type == NODE_NAME) {
      ...
      if (all_children_have_type(lhs_node, NODE_NAME)) {
        ...
        */
        ...
        SENIE_LOG("todo: push each item onto stack using nth");
      }

    } else {
      SENIE_ERROR("compile_define lhs should be a name or a list");
    }
...
void compile_address_of(senie_node* ast, senie_program* program) {
  ...
  if (fn_name->type != NODE_NAME) {
    SENIE_ERROR("compile_address_of given non-function-name argument");
    return;
  }
...
void compile_fn_call(senie_node* ast, senie_program* program) {
  ...
  if (invocation->type != NODE_LIST) {
    SENIE_ERROR("compile_fn_call given non-list to invoke");
    return;
  }

  warn_if_alterable("compile_fn_call invocation", invocation);

  senie_node* fn_info_index = safe_first(invocation->value.first_child);
  ...
  // invocation
  senie_node* args = safe_next(fn_info_index); // pairs of label/value declarations
  while (args != NULL) {
    senie_node* label = args;
...
void compile_vector_append(senie_node* ast, senie_program* program) {
  ...
  if (vector->type == NODE_NAME) {
    ...
    SENIE_ERROR("compile_vector_append: can't find local or global variable");
  }
}
...
void compile_loop(senie_node* ast, senie_program* program) {
  ...
  senie_node* name_node = safe_first(parameters_node->value.first_child);

  senie_node* from_node      = NULL;
  senie_node* to_node        = NULL;
  senie_node* upto_node      = NULL;
  senie_node* increment_node = NULL;
  bool        have_from      = false;
  bool        have_to        = false;
  bool        have_upto      = false;
  bool        have_increment = false;

  senie_node* node = name_node;
  ...
  if (have_to == false) {
    if (have_upto == false) {
      SENIE_ERROR("step form requires either a 'to' or 'upto' parameter");
      return;
    }
  } else {
    use_to = true;
...
void compile_fence(senie_node* ast, senie_program* program) {
  ...
  senie_node* parameters_node = safe_next(ast);
  if (parameters_node->type != NODE_LIST) {
    SENIE_ERROR("expected a list that defines fence parameters");
    return;
  }
...
void register_top_level_fns(senie_node* ast, senie_program* program) {
  ...
  while (ast != NULL) {
    ...
    senie_node* name_and_params = safe_next(fn_keyword);
    if (name_and_params->type != NODE_LIST) {
      ast = safe_next(ast);
      continue;
    }

    senie_node* name       = safe_first(name_and_params->value.first_child);
  ...
  for (i32 i = 0; i < program->code_size; i++) {
    ...
    if (bc->op == CALL) {
      ...
      if (offset->op != LOAD && offset->arg0.value.i != MEM_SEG_CONSTANT) {
        SENIE_ERROR("correct_function_addresses expected a 'LOAD CONST' 2 "
      ...
      offset = bc - 1;
      if (offset->op != LOAD && offset->arg0.value.i != MEM_SEG_CONSTANT) {
        SENIE_ERROR("correct_function_addresses expected a 'LOAD CONST' 1 "
...
void compile_fn_invocation(senie_node* ast, senie_program* program, i32 fn_info_index) {
  ...
  // overwrite the default arguments with the actual arguments given by the fn
  // invocation
  senie_node* args = safe_next(ast); // pairs of label/value declarations
  while (args != NULL) {
    senie_node* label   = args;
    i32         label_i = get_node_value_i32(label);
  ...
  if (ast->type == NODE_LIST) {
    if (alterable(ast) && is_node_colour_constructor(ast)) {
      ...
      if (alterable(ast)) {
        ...
        SENIE_ERROR("given an alterable list that wasn't a colour constructor???");
      }
...
bool is_list_beginning_with(senie_node* ast, i32 index) {
  if (ast->type != NODE_LIST) {
    return false;
  }

  senie_node* keyword = safe_first(ast->value.first_child);
...
compile_program_common(senie_node* ast, i32 program_max_size, senie_word_lut* word_lut) {
  ...
  // compile the global defines common to all senie programs
  // (e.g. canvas/width)
...
senie_program* compile_program_with_genotype(senie_node*     ast,
  ...
  if (all_genes_assigned == false) {
    SENIE_ERROR("not all genes were assigned");
    return NULL;
  }
