...
sen_uv_mapping** g_brush_info    = NULL;
...
void allocate_uv_mapping(sen_brush_type type,
                         i32            sub_type,
                         i32            min_x,
                         i32            min_y,
                         i32            max_x,
...
void uv_mapper_subsystem_startup() {
  g_brush_info = (sen_uv_mapping**)calloc(NUM_BRUSHES, sizeof(sen_uv_mapping*));
...
void free_uv_mapping(sen_brush_type type) {
  sen_uv_mapping* m   = g_brush_info[type];
  i32             num = num_uv_mappings[type];

  for (i32 i = 0; i < num; i++) {
    sen_uv_mapping* p = &(m[i]);
    free(p->map);
  }
