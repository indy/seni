...
export default class RenderPacket {
  constructor() {
    ...
    this.textureItemSize = textureItemSize;
    ...
    this.abTexture = new ArrayBuffer(textureItemSize * f32Size * bufferSize);

    this.vertexBuffer = new Float32Array(this.abVertex);
    this.colourBuffer = new Float32Array(this.abColour);
    this.textureBuffer = new Float32Array(this.abTexture);
  ...
  addVertex(p, c, uv) {
    ...
    this.textureBuffer[this.textureBufferLevel + 0] = uv[0];
    this.textureBuffer[this.textureBufferLevel + 1] = uv[1];

    this.bufferLevel += 1;
    this.vertexBufferLevel += vertexItemSize;
    this.colourBufferLevel += colourItemSize;
    this.textureBufferLevel += textureItemSize;
  }
}
