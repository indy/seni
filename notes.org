* TODO [1/9]
  1. [ ] calling native built-in functions
  2. [ ] remove seni_common_args.h when interpreter is deleted
  3. [X] default all numbers to float
  4. [ ] manipulate vectors
  5. [ ] special fn: on-matrix-stack
  6. [ ] why does setq seg fault when it returns NULL?
  7. [ ] 'in' paramaeter for loop ?
  8. [ ] constants like canvas/width
  9. [ ] how would a stack machine compile first class functions?
