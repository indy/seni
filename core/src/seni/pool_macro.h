...
//   sen_item_pool : structure used to manage the pool
//
//   struct sen_item_pool *item_pool_allocate(i32 num_slabs,
//                                             i32 slab_size,
//                                             i32 max_slabs_allowed);
//   void item_pool_free(struct sen_item_pool *item_pool);
//   sen_item *item_pool_get(struct sen_item_pool *item_pool);
//   void item_pool_return(struct sen_item_pool *item_pool, sen_item *item);
//
// *** RECOMMENDATIONS ***
//
// have global level pool structures:
//
//   struct sen_item_pool *g_item_pool;
//
...
//
//   sen_item *item_get_from_pool()
//   {
//     sen_item *item = item_pool_get(g_item_pool);
//     return item;
//   }
//
//   void item_return_to_pool(sen_item *item)
//   {
...
#define SEN_POOL(ITEM, ITEM_NAME)                                                             \
  struct ITEM##_slab {                                                                        \
    ITEM*               ITEM_NAME##s;                                                         \
    i32                 slab_size;                                                            \
  ...
  extern struct ITEM##_pool* ITEM_NAME##_pool_allocate(i32 num_slabs, i32 slab_size,          \
                                                       i32 max_slabs_allowed);                \
  extern ITEM*               ITEM_NAME##_pool_get(struct ITEM##_pool* ITEM_NAME##_pool);      \
  extern void                ITEM_NAME##_pool_return(struct ITEM##_pool* ITEM_NAME##_pool,    \
                                      ITEM*               ITEM_NAME);                        \
                                                                                              \
  ...
  bool ITEM_NAME##_pool_add_slab(struct ITEM##_pool* ITEM_NAME##_pool) {                      \
    if (ITEM_NAME##_pool->num_slabs >= ITEM_NAME##_pool->max_slabs_allowed) {                 \
      ...
      SEN_ERROR("will not allocate more than %d %s", ITEM_NAME##_pool->max_slabs_allowed,     \
                slab_name);                                                                   \
      return false;                                                                           \
    }                                                                                         \
                                                                                              \
    struct ITEM##_slab* ITEM_NAME##_slab =                                                    \
        ITEM_NAME##_slab_allocate(ITEM_NAME##_pool->slab_size);                               \
  ...
  void ITEM_NAME##_pool_free(struct ITEM##_pool* ITEM_NAME##_pool) {                          \
    ...
    for (i32 i = 0; i < ITEM_NAME##_pool->num_slabs; i++) {                                   \
      if (ITEM_NAME##_slab) {                                                                 \
        ...
        SEN_ERROR("slab unexpectedly null");                                                  \
        break;                                                                                \
      }                                                                                       \
    ...
    free(ITEM_NAME##_pool);                                                                   \
                                                                                              ...
                                                                                              \
  struct ITEM##_pool* ITEM_NAME##_pool_allocate(i32 num_slabs, i32 slab_size,                 \
                                                i32 max_slabs_allowed) {                      \
  ...
  ITEM* ITEM_NAME##_pool_get(struct ITEM##_pool* ITEM_NAME##_pool) {                          \
    if (ITEM_NAME##_pool->available == NULL) {                                                \
      if (!ITEM_NAME##_pool_add_slab(ITEM_NAME##_pool)) {                                     \
        const char* slab_name = #ITEM_NAME "_slabs";                                          \
        SEN_ERROR("cannot add more than %d %s", ITEM_NAME##_pool->max_slabs_allowed,          \
                  slab_name);                                                                 \
        return NULL;                                                                          \
      }                                                                                       \
    }                                                                                         \
  ...
  void ITEM_NAME##_pool_pretty_print(struct ITEM##_pool* ITEM_NAME##_pool) {                  \
    ...
    SEN_LOG("%s_pool:"                                                                        \
            ...
            item_name, ITEM_NAME##_pool->slab_size, ITEM_NAME##_pool->num_slabs,              \
            ITEM_NAME##_pool->max_slabs_allowed, ITEM_NAME##_pool->get_count,                 \
            ITEM_NAME##_pool->return_count, ITEM_NAME##_pool->high_water_mark,                \
            ITEM_NAME##_pool->current_water_mark);                                            \
  }
