...
export i32 create_initial_generation(i32 population_size, i32 seed) {
  ...
  if (g_genotype_list == NULL) {
    trait_list_return_to_pool(trait_list);
    SENI_ERROR("create_initial_generation: "
  ...
  if (count != population_size) {
    trait_list_return_to_pool(trait_list);
    SENI_ERROR("create_initial_generation: population size mismatch %d "
               "requested, %d created",
               population_size,
               count);
    return 0;
...
export void genotype_move_to_buffer(i32 index) {
  ...
  if (res == false) {
    SENI_ERROR("genotype_move_to_buffer: genotype_serialize returned false "
               "(for index %d)",
               index);
  }
...
next_generation_build(i32 parent_size, i32 population_size, f32 mutation_rate, i32 rng) {
  ...
  if (count != parent_size) {
    SENI_ERROR("next_generation_build: parent_size (%d) mismatch with "
               "genotypes given (%d)",
               parent_size,
               count);
    return false;
