* TODO [3/19]
  1. [ ] get high-res render working
  2. [ ] impl: initial seed
  3. [ ] add an END token to serialized output, use as a sanity check
  4. [ ] add unit tests for compiling with a genotype
  5. [ ] pretty print program emits names
  6. [ ] better seni_ga.c::hack_node_to_var (lists, 2d etc)
